...
4. **Docker Compose File Path**: `docker-compose.production.yml`
...
Coolify should automatically detect the volumes from docker-compose.production.yml:
...
```bash
docker compose -f docker-compose.production.yml exec backend bench list-sites
```
...
```bash
docker compose -f docker-compose.production.yml exec mariadb mysqladmin ping -p
```
...
```bash
docker compose -f docker-compose.production.yml restart configurator
```
...
2. Increase MariaDB buffer pool (in docker-compose.production.yml):
```yaml
...
```bash
docker compose -f docker-compose.production.yml logs frontend | grep -i ssl
```
